...
for file, pct_loss in significant_losses:
    ...
    if rows_lost - legitimate_removals > rows_lost * 0.1:  # >10% unexplained
        results.append(f"\n⚠️  RECOMMENDATION: RESTORE - Significant unexplained data loss!")
        restore_recommendations.append(file)
    elif pct_loss > 40 and rows_lost > legitimate_removals:
        results.append(f"\n⚠️  RECOMMENDATION: REVIEW - High loss rate, needs manual review")
        restore_recommendations.append(file)
    else:
        results.append(f"\n✓ RECOMMENDATION: KEEP AS IS - Loss appears justified (duplicates/nulls)")
...
if restore_recommendations:
    results.append(f"\n⚠️  FILES RECOMMENDED FOR RESTORATION: {len(restore_recommendations)}")
    for file in restore_recommendations:
        results.append(f"   - {file}")
else:
    results.append("\n✓ All data losses appear justified - no restoration needed")
...
print("\n✓ Investigation report saved to data_loss_investigation.txt")

# Output files to restore for script usage
if restore_recommendations:
